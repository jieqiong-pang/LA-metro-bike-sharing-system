  ...
  <div class="dv_heatmap_map_container">
    ...
    <div class="map-overlay top">
      <div class="map-overlay-inner">
        <h2 class="map-overlay-inner-header">Quarterly Station Popularity Heatmap</h2>
        <b><label id="quarter"></label></b>
        <input id="slider" type="range" min="0" max="15" step="1" value="0" />
      </div>
      <div class="map-overlay-inner">
        <div id="legend" class="legend">
          <div class="bar"></div>
          <div><b>Popularity</b></div>
        </div>
      </div>
      <div class="map-overlay-inner">
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/thumb/8/88/Map_marker.svg/20px-Map_marker.svg.png"
        />
        <b>Most Popular Stations</b>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      data: null,
      area_setup: {
        'Overview': { 'center': [-118.26, 33.97], 'zoom': 9 },
        'DTLA': { 'center': [-118.26, 34.06], 'zoom': 11.4 },
        'Pasadena': { 'center': [-118.16, 34.15], 'zoom': 12 },
        "Port of LA": { 'center': [-118.26, 33.74], 'zoom': 11.8 },
        'Westside': { 'center': [-118.43, 34], 'zoom': 11.8 },
        "North Hollywood": { 'center': [-118.376, 34.16], 'zoom': 12.6 },
      },
      quarters: [
        "2016-q3",
        "2016-q4",
  ...
  methods: {
    ...
    initMap() {
      ...
      this.map = new mapboxgl.Map({
        ...
        // center: area_setup[this.selected_area]['center'],
        // zoom: area_setup[this.selected_area]['zoom'],
        center: this.area_setup["Overview"]["center"],
        zoom: this.area_setup["Overview"]["zoom"],
      });
      const that = this;
      const map = this.map;
      ...
      map.on("load", function() {
        map.loadImage(
          ...
          function(error, image) {
            ...
            map.setLayoutProperty("heatmap-marker-0", "visibility", "visible");

            map.setLayoutProperty("heatmap-heat-0", "visibility", "visible");

            map.setLayoutProperty("heatmap-point-0", "visibility", "visible");
    ...
    changeRegion() {
      ...
      this.map.jumpTo({ 'center': this.area_setup[this.selected_area]['center'], 'zoom': this.area_setup[this.selected_area]['zoom'] });
    },
  },

  watch: {
    selected_area(newV, oldV) {
      // console.log(newV)
      if (newV !== null) {
        this.changeRegion();
      }
